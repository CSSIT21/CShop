...
export class CartService {
  ...
  async updateamount(userID: number, addressID: number ,totalprice:number, newamount : {id:number,amount:number}[]){
    const prisma = this.prisma
    await Promise.all(newamount.map(async(data)=>{
      await prisma.order_cart_item.update({where:{id:data.id},data:{quantity: data.amount }})
    }))

  

    const orders = await prisma.order_cart_item.findMany({where:{customer_id:userID},include:{product_id_from_order_cart_item:true,} })
    const address = await prisma.address.findUnique({where:{id:addressID}})
    const trackingdata = await this.generateTrackingNumber({province:address.province,recipient_name:address.recipient_name})
    const status = await prisma.order_status.create({data:{status_id:trackingdata.id}})
    await prisma.order.create({data:{total_price:totalprice,order_date: new Date(),customer_id:userID,status:"Received_a_request"}})
