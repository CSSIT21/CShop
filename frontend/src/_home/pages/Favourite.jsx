...
const FavouritePage = () => {
    ...
    if (isLoggedIn) {
      setProducts((products) => {
        const target = products.find(
          (e) => e.product_id_from_wishlist.id == id
        );
        const { product_id_from_wishlist: product } = target;

        if (product.customer_wishlist.length > 0)
          product.customer_wishlist.pop();
        else
          product.customer_wishlist = [
  ...
  const getData = () => {
    if (isLoggedIn) {
      axios
        .get(
          `${config.SERVER_URL}/home/favorites/${user.id}?take=16&skip=${
            (page - 1) * 16
          }`
        )
        .then(({ data }) => {
        ...
        {products.length > 0 ? (
          <>
            <Grid container spacing={10} mb={5}>
              <For
                each={products}
                children={({ product_id_from_wishlist: product, id }) => (
